...
class DQNAgent:
    ...
    def train(self, terminal_state, phase, step):
        ...
        # self.rest2_time += timer()-tstart
        # tstart = timer()
        # Get future states from minibatch, then query NN model for Q values
        # When using target network, query it, otherwise main network should be queried
        new_current_states = np.array([transition[3] for transition in minibatch], dtype=object)
        new_phase_list = np.array([transition[6] for transition in minibatch])
        # future_qs_list with length of minibatch
        future_qs_list = []
        # self.rest3_time += timer()-tstart
        # tstart = timer()
        ...
        for index, (current_state, action, reward, new_current_state, new_field, new_move, new_phase, done) in enumerate(minibatch):
            ...
            if not done:
                validIdx = getPossibleMovesIndex(new_phase, self.player, new_field, new_move)
...
################################# Training class ################################
class DQNTraining:
    episode_reward = {PLAYER1:None, PLAYER2:None}
    step = None
    ...
    def __init__(self, initParams=None):
        ...
        self.agents = {PLAYER1:self.agent1, PLAYER2:self.agent2}
    ...
    def startGame(self):
        self.episode_reward = {PLAYER1:0, PLAYER2:0}
        self.step = 1
        ...
        validIdx = getPossibleMovesIndex(self.phase, self.player, self.field, self.move) # start in phase set, all set moves possible

        # This part stays mostly the same, the change is to query a model for Q values
        if np.random.random() > self.epsilon:
            # Get action from Q table
            qVals = self.agents[self.player].get_qs(self.inputVector, self.phase)
        ...
        newField = getMoveFromIndex(self.phase, self.player, self.field, selectedAction) # start in phase set, new field with 1 token from player 1 
    ...
    def stepGame(self):
        validIdx = getPossibleMovesIndex(self.phase, self.player, self.field, self.move)
        ...
        newField = getMoveFromIndex(self.phase, self.player, self.field, selectedAction)
        ...
        # Count reward
        self.episode_reward[self.player] += reward

        # Every step we update replay memory and train main network
        self.agents[self.player].update_replay_memory((self.lastInputVector[self.player], self.lastSelectedAction[self.player], reward, self.inputVector, self.field, self.move, self.phase, done), self.lastPhase[self.player])
        self.agents[self.player].train(done, self.lastPhase[self.player], self.step)
    ...
    def finishGame(self):
        self.ep_rewards_1 += [self.episode_reward[self.agent1.getPlayer()]]
        self.ep_rewards_2 += [self.episode_reward[self.agent2.getPlayer()]]
    ...
    def setPlayerAgent1(self, player):
        self.agent1.setPlayer(player)
        self.agent2.setPlayer(PLAYER1+PLAYER2-player)
        self.agents[player] = self.agent1
        self.agents[PLAYER1+PLAYER2-player] = self.agent2
    ...
    def switchPlayers(self):
        ...
        self.setPlayerAgent1(self.agent2.getPlayer())
